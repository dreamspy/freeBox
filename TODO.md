...
- [ ] **Investigate freeMac LaunchAgent context auth failure** (2026-05-20). `claude remote-control --spawn=same-dir --name fm-...` succeeds when launched from an interactive ssh shell but fails with `401 — Remote Control is only available with claude.ai subscriptions` when launched from the `com.freebox.mac-tmux-ensure` LaunchAgent. Same binary, same credentials file, same args, same `HOME`/`USER`/`PATH`. Verified: account is `claude_max` subscription with valid OAuth token. **Env vars ruled out** (2026-05-20): running with `env -i HOME=... USER=... PATH=... XPC_FLAGS=1 XPC_SERVICE_NAME=com.freebox.mac-tmux-ensure` from an ssh shell still succeeded, so the launchd context's minimal env isn't the trigger. The variable must be something about the parent process identity / macOS security session (launchd-spawned child vs sshd-spawned child). **Workaround in place:** manually launched 8 tmux sessions from an interactive ssh shell on 2026-05-20 ~11:11; verified all 8 stayed alive ≥10 minutes with claude reconnecting cleanly after a transient network outage. Per-vault respawn logs at `~/Library/Logs/freemac-vaults/<vault>.log`. **Next probes:** (a) TCC/sandbox — check `tccutil` and Full Disk Access for `bash`/`claude`; (b) reproduce with `sudo launchctl asuser <uid> claude remote-control ...` to confirm launchd-context (not the script wrapper) is the variable; (c) `claude --debug` to see the auth code path; (d) snoop the TLS request bodies (e.g. via mitmproxy) to compare what client info each context sends.
