...
- **File versioning (Mac):** Staggered file versioning enabled on the Mac peer. Deleted/modified files are kept in `/Users/frimann/Vaults-syncthing-versioning` (custom path, outside the synced folder). Versions are not synced between peers (`.stversions` is in `.stignore`). Useful as a safety net while testing sync/delete workflows.
- **rsync snapshots (Mac):** launchd agent `com.frimann.vaults-rsync-backup` takes a hardlink-based rsync snapshot of `~/Vaults/` to `/Users/frimann/Vaults-rsync-versioning/` every 5 minutes. Snapshots older than 365 days are pruned automatically. Plist at `~/Library/LaunchAgents/com.frimann.vaults-rsync-backup.plist`. Manage with `launchctl load/unload`.
